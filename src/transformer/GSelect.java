...
public class GSelect extends GTransformer {
	...
	public GShape groupingRect(Vector<GShape> shapeVector) {
		...
		for (GShape shape : shapeVector) {
			minx = shape.getMin().x < minx ? shape.getMin().x : minx;
			miny = shape.getMin().y < miny ? shape.getMin().y : miny;
			maxx = shape.getMax().x > maxx ? shape.getMax().x : maxx;
			maxy = shape.getMax().y > maxy ? shape.getMax().y : maxy;
		}
